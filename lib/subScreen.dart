...
class _SubscreenState extends State<Subscreen> {
  ...
  void _listenToPurchaseUpdated(
    ...
    for (var purchaseDetails in purchaseDetailsList) {
      print('Received purchase: ${purchaseDetails.productID}, '
      ...
      if (purchaseDetails.status == PurchaseStatus.pending) {
        ...
        _showSnackBar('Purchase restored: ${purchaseDetails.productID}');
        continue; // skip saving restored purchases
      } else if (purchaseDetails.status == PurchaseStatus.purchased) {
        if (purchaseDetails.pendingCompletePurchase) {
          await _inAppPurchase.completePurchase(purchaseDetails);
        }
  ...
  void _handleBuyNow(BuildContext context, String productId) async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text('Do you want to purchase the "$productId" plan?'),
        actions: [
          TextButton(
            child: Text('Cancel'),
